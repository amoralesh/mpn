...
@section('titulo', 'Tipo Alarma')
@section('dependencia', ' - Policía Preventiva - ')


@section('directorioTitle', 'Tipo Alarma') 
@section('directorio')
    <li class="breadcrumb-item ">Tipo Alarma</li>
@endsection  
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo de alarma</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="tAlarmaDT">
            <thead>
                <tr>
					<th>#</th>
					<th>Tipo Alarma</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipoAlarmas/create";
			e.stopPropagation();
		});
	...
	function listAllTipoAlarma(){
	
		var table = $('#tAlarmaDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipoAlarmas",
		    				"type" : "GET",
								"dataSrc": "",  
				...
				$('#tAlarmaDT tbody').on('click', '#editar', function(e) {
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipoAlarmas/" + id + "/edit";
					e.stopPropagation();
				});
