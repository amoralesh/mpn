...
@section('titulo', 'Motivo Alarma')
@section('dependencia', ' - Policía Preventiva - ')


@section('directorioTitle', 'Motivo Alarma') 
@section('directorio')
    <li class="breadcrumb-item ">Motivo Alarma</li>
@endsection
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo motivo de alarma</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="mADT">
            <thead>
                <tr>
					<th>#</th>
					<th>Motivo Alarma</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/motivoAlarmas/create";
			e.stopPropagation();
		});

		listAllMotivoAlarmas();  

	});

	function listAllMotivoAlarmas(){
		
		var table = $('#mADT').DataTable({
						...
						"processing" : true, 
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/motivoAlarmas",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#mADT tbody').on('click', '#editar', function(e) {
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/motivoAlarmas/" + id + "/edit";
					e.stopPropagation();
				});
				//editar
