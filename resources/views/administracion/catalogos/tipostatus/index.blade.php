...
@section('directorio')
    <li class="breadcrumb-item ">Tipo Status</li>
@endsection  
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo de estatus</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="statusDT">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Status</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipostatus/create";
			e.stopPropagation();
		});

		listAllNuevoTipoStatus();

 	});

	function listAllNuevoTipoStatus(){
		
		var table = $('#statusDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipostatus",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#statusDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipostatus/" + id + "/edit";
					e.stopPropagation();
				});
	}
