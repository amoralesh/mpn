...
@section('titulo', 'Tipo Giro')
@section('dependencia', ' - Policía Preventiva - ')


@section('directorioTitle', 'Tipo Giro') 
@section('directorio')
    <li class="breadcrumb-item ">Tipo Giro</li>
@endsection
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo giro</button>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipoGiros/create";
			e.stopPropagation();
		});
	...
	function listAllTipoGiro(){
		...
		var table = $('#TipoGirossDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipoGiros",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#TipoGirossDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipoGiros/" + id + "/edit";
					e.stopPropagation();
				});
