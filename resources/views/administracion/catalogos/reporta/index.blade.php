...
@section('titulo', 'Reporta')
@section('dependencia', ' - Policía Preventiva - ')

@section('directorioTitle', 'Reporta') 
@section('directorio')
    <li class="breadcrumb-item ">Reporta</li>
@endsection  
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nueva entidad que reporta</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="reportaDT">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Reporta</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/reporta/create";
			e.stopPropagation();
		});
	...
	function listAllReporta(){
		 
		var table = $('#reportaDT').DataTable({
			responsive: true,
			"autoWidth": false,
			"deferRender": true,
			"bProcessing": true,
			...
			//autoFill: true,
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/reporta",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#reportaDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/reporta/" + id + "/edit";
					e.stopPropagation();
				});
