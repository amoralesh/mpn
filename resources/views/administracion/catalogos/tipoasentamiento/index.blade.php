...
@section('titulo', 'Tipo Asentamiento')
@section('dependencia', ' - Policía Preventiva - ')


@section('directorioTitle', 'Tipo Asentamiento') 
@section('directorio')
    <li class="breadcrumb-item ">Tipo Asentamiento</li>
@endsection  
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo de asentamiento</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="tipoAsentamientoDT">
            <thead>
                <tr>
                    <th>#</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipoAsentamientos/create";
			e.stopPropagation();
		});

		listAllTipoAsentamiento();

 	});

	 function listAllTipoAsentamiento(){
		 
		var table = $('#tipoAsentamientoDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipoAsentamientos",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#tipoAsentamientoDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipoAsentamientos/" + id + "/edit";
					e.stopPropagation();
				});
	 }
