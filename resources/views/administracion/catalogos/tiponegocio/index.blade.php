...
@section('titulo', 'Tipo Negocio')
@section('dependencia', ' - Policía Preventiva - ')


@section('directorioTitle', 'Tipo Negocio') 
@section('directorio')
    <li class="breadcrumb-item ">Tipo Negocio</li>
@endsection    
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo de negocio</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="TipoNegociosDT">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Tipo Negocio</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipoNegocios/create";
			e.stopPropagation();
		});
	...
	function listAllTipoNegocio(){
		

		var table = $('#TipoNegociosDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipoNegocios",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#TipoNegociosDT tbody').on('click', '#editar', function(e)
				{
					var current_row = $(this).parents('tr');
				    if (current_row.hasClass('child')) { current_row = current_row.prev(); }
    				var data = table.row(current_row).data();
					var id = data.id;
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipoNegocios/" + id + "/edit";
					e.stopPropagation();
				});
