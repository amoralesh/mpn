...
@section('directorioTitle', 'Participación')  
@section('directorio')
    <li class="breadcrumb-item ">Participación</li>
@endsection   
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nueva participacion</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="participacionesDT">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Participación</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/participaciones/create";
			e.stopPropagation();
		});
	...
	function listAllParticipaciones(){
		
		var table = $('#participacionesDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/participaciones",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#participacionesDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/participaciones/" + id + "/edit";
					e.stopPropagation();
				});
		}
