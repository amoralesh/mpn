...
@section('titulo', 'Tipo Dispositivo')
@section('dependencia', ' - Policía Preventiva - ')



@section('directorioTitle', 'Tipo Dispositivo') 
@section('directorio')
    <li class="breadcrumb-item ">Tipo Dispositivo</li>
@endsection  
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo de dispositivo</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="TipoDispositivosDT">
            <thead>
                <tr>
                    <th>#</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipoDispositivo/create";
			e.stopPropagation();
		});

		listAllTipoDeDispositivo();

 	});


	function listAllTipoDeDispositivo(){
		

		var table = $('#TipoDispositivosDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipoDispositivo",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#TipoDispositivosDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipoDispositivo/" + id + "/edit";
					e.stopPropagation();
				});
	}
