...
@section('titulo', 'Tipo Giro General')
@section('dependencia', ' - Policía Preventiva - ')



@section('directorioTitle', 'Tipo Giro General') 
@section('directorio')
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo tipo giro general</button>
    
    <section style="margin-top:20px;"> 
        <table class="table table-striped table-bordered table-hover" id="TipoGirossDT">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Tipo General</th>
...
<script>
    $(function() {
		...
		$('#crear').on('click', function(e){
			document.location.href = "{{ url('/') }}/administracion/catalogo/tipogirosgenerales/create";
			e.stopPropagation();
		});

		listAllTipoGiroGeneral();

	});



	function listAllTipoGiroGeneral(){
		
		var table = $('#TipoGirossDT').DataTable({
						...
						"processing" : true,
						"ajax"   :  {
		    				"url"  : "{{ url('/') }}/administracion/rest/catalogo/tipogirosgenerales",
		    				"type" : "GET",
								"dataSrc": "",
				...
				$('#TipoGirossDT tbody').on('click', '#editar', function(e){
					...
					document.location.href = "{{ url('/') }}/administracion/catalogo/tipogirosgenerales/" + id + "/edit";
					e.stopPropagation();
				});
