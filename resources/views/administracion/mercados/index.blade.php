...
@section('directorioTitle', 'Mercados')
@section('directorioTitle', 'Mercados') 
@section('directorio')  
    <li class="breadcrumb-item ">Mercados</li>
@endsection  
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nuevo Mercados</button>
...
<script>
    $(function() {
        ...
        $('#crear').on('click', function(e) {
            document.location.href = "{{ url('/') }}/administracion/mercados/create";
            e.stopPropagation();
        });
    ...
    function listAllEstablecimientos(){
        ...
        var table = $('#negociosDT').DataTable({
            ...
            "processing": true,
            "ajax": {  
                "url": "{{ url('/') }}/administracion/rest/mercados",
                "type": "POST",  
                "data": {
                    _token: token
                },
        ...
        $('#negociosDT tbody').on('click', '#editar', function(e) {
            ...
            var data = table.row(current_row).data();
            document.location.href = "{{ url('/') }}/administracion/mercados/" + data.id + "/edit";
            e.stopPropagation();
        });
                ...
                preConfirm: function (motivo) {
                    return new Promise(function (resolve, reject) {
                        setTimeout(function() {
                            ...
                            $.ajax({
                                url: "{{ url('/') }}/administracion/rest/mercados/habilitar",
                                type: "POST",
                                data: { 
                ...
                preConfirm: function (motivo) {
                    return new Promise(function (resolve, reject) {
                        setTimeout(function() {
                            ...
                            $.ajax({
                                url: "{{ url('/') }}/administracion/rest/mercados/deshabilitar",
                                type: "POST",
                                data: { 
                                    id: idEstablecimiento,
                                    motivoAltaBaja: motivo
    ...
    function findEstablecimientoById( data ) {      
        ...
        {   
            $.ajax({         
                url: "{{ url('/') }}/administracion/rest/dispositivos/mercados/" + data.id,
                type: "GET",       
                dataType : 'json',   
                beforeSend: function( xhr ) {
                    swal({ title: 'Recuperando Información' });
                ...
                success: function ( dispositivos ) {
                    ...
                    var dispositivosInfo =
                        '<table class="table table-striped table-bordered table-hover" id="dispositivosDT">'+
                            ...
                            jQuery.each( dispositivos , function( indice , dispositivo ) { 
                                ...
                                : '<div onclick="real(\''+dispositivo.etiqueta+'\','+dispositivo.id+')" class="" style="color: rgba(255,80,80,1);"><button type="button" class="btn btn-primary" style="width:150px; margin-top:2px"><i class="fa fa-times-circle"></i> A real</button></div>';
                    ...
                    bootbox.dialog({
                        title: "DISPOSITIVOS POR PUERTA DE MARCADO : "+data.nombre,
                        message: informacion,
                        buttons: {
                            success: {
                                label: "Cerrar",
