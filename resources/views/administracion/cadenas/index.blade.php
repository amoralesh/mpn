...
@section('titulo', 'Cadenas')
@section('dependencia', ' - Policía Preventiva - ')


@section('directorioTitle', 'Cadenas')
@section('directorio')
    <li class="breadcrumb-item ">Cadenas</li>
@endsection 
...
<div style="margin: 25px auto; width:95%;">
    ...
    <button type="button" id="crear" class="btn btn-primary">Crear nueva cadena</button>
...
<script>
    ...
    $(function() {
        ...
        $('#crear').on('click', function(e) {
            document.location.href = "{{ url('/') }}/administracion/cadenas/create";
            e.stopPropagation();
        });
 
 
        $('#cadenasDT tbody').on('click', '#encargados', function(e) {
            var current_row = $(this).parents('tr');
            if (current_row.hasClass('child')) {
                current_row = current_row.prev();
            }
            var data = table.row(current_row).data(); 
            findEncargadosByIdCadena( data );  
            e.stopPropagation();
        }); 
 
 
        $('#cadenasDT tbody').on('click', '#asociaciones', function(e) {
            var current_row = $(this).parents('tr');
            if (current_row.hasClass('child')) {
                current_row = current_row.prev();
            }
            var data = table.row(current_row).data();
            findAsociacionesByIdCadena( data );  
            e.stopPropagation();
        });

  
        $('#cadenasDT tbody').on('click', '#informacion', function(e) {
            var current_row = $(this).parents('tr');
            if (current_row.hasClass('child')) {
                current_row = current_row.prev();
            }
        ...
        $('#cadenasDT tbody').on('click', '#editar', function(e) {
            var current_row = $(this).parents('tr');
            if (current_row.hasClass('child')) {
                current_row = current_row.prev();
            }
            ...
            document.location.href = "{{ url('/') }}/administracion/cadenas/" + id + "/edit";
            e.stopPropagation();
        });

        
        // Handle click on checkbox
        $('#cadenasDT tbody').on('click', '#alta', function(e) {
            var current_row = $(this).parents('tr');
            if (current_row.hasClass('child')) {
                current_row = current_row.prev();
            }
            ...
            var idCadena = data.id; 
            var nombre = data.nombre;

            swal({
                title: 'Habilitar Cadena ?',
                text: 'La cadena ' + nombre + ' ya podrá ser editada y se le podrá asignar con nuevos establecimientos',
                input: 'textarea',
                type: 'warning',
                ...
                preConfirm: function (motivo) {
                    return new Promise(function (resolve, reject) {
                        setTimeout(function() {
                            ...
                            $.ajax({
                                url: "{{ url('/') }}/administracion/rest/cadenas/habilitar",
                                type: "POST",
                                data: { 
                                    id: idCadena,
                                ...
                                dataType: "html",
                                success: function () {
                                    swal('Habilitado!','La cadenas ha sido habilitada.','success')
                                    table.ajax.reload();
                                },
        ...
        // Handle click on checkbox
        $('#cadenasDT tbody').on('click', '#baja', function(e) {
            var current_row = $(this).parents('tr');
            if (current_row.hasClass('child')) {
                current_row = current_row.prev();
            }
            ...
            swal({
                title: 'Deshabilitar Cadena ?',
                text: 'La cadena ' + nombre + ' ya no podrá ser editada y no se podrá asignar con nuevos establecimientos ni cadenas',
                input: 'textarea',
                type: 'warning',
                showCancelButton: true,
                ...
                preConfirm: function (motivo) {
                    return new Promise(function (resolve, reject) {
                        setTimeout(function() {
                            ...
                            $.ajax({
                                url: "{{ url('/') }}/administracion/rest/cadenas/deshabilitar",
                                type: "POST", 
                                data: { 
                                    id: idAsociacion,
                                    motivoAltaBaja: motivo
                                ...
                                dataType: "html",
                                success: function () {
                                    swal('Deshabilitado!','La cadena ha sido deshabilitada.','success')
                                    table.ajax.reload();
                                },
                                error: function (xhr, ajaxOptions, thrownError) {
        ...
        function findCadenaById( data ){

            $.ajax({   
                url: "{{ url('/') }}/administracion/rest/cadenas/findById/" + data.id,
                type: "GET",      
                dataType : 'json', 
        ...
        function findAsociacionesByIdCadena( data ){

            $.ajax({   
                url: "{{ url('/') }}/administracion/rest/asociaciones/cadena/" + data.id,   
                type: "GET",      
                dataType : 'json', 
                beforeSend: function( xhr ) {
                    swal({ title: 'Recuperando Información' });
                ...
                success: function ( asociacion ) { 
                    ...
                    informacion += 
                        "<p style='padding-left: 50px;'>"+ 
                            "<strong>Nombre:</strong> " + asociacion.etiqueta + 
                            "</br>" +
                            "<strong>Alias:</strong> " + asociacion.alias +
                            "</br>" +
                            "<strong>Fecha de alta:</strong> " + asociacion.fechaAlta.date +
                            "</br>" +
                            "<strong>Status:</strong> " + asociacion.status +
                            "</br>" +
                            "<strong>Motivos de alta y baja:</strong> " + motivosAltaBaja + 
                        "</p>";
        ...
        function findEncargadosByIdCadena( data ){ 
            $.ajax({   
                url: "{{ url('/') }}/administracion/rest/encargados/cadena/" + data.id,
                type: "GET",       
                dataType : 'json', 
                beforeSend: function( xhr ) {
                    swal({ title: 'Recuperando Información' });
