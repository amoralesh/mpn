...
<script>
    ...
    $('.sideBar-body').on("click", function() {
        ...
        if( typeOfUser == "Admin" ){
            getMensajesAdmin("{{  Auth::user()->getEmail() }}", emailElemento, 0, dialog);
        }
    ...
    $('.moreMesagges').on("click", function() {
        ...
        if (row != null) {
           ...
           if( typeOfUser == "Admin" ){
                //*Limpiamos mensajes de otro chat antes de cargar otro*/
                $(".message-body").empty();
                getMensajesAdmin("{{  Auth::user()->getEmail() }}", emailElemento,row, dialog);
            } 
    ...
    $(function() {
        ...
        $('#send').on('click', function(e) {
            ...
            if (texto != "") {
                if( typeOfUser == "Admin" ){
                    addMessageAdmin(texto, emailElemento);
                } 
                $('#conversation').scrollTop(1000);
                $('#comment').val('');
            }
    ...
    function addMessageAdmin(text, emailReceptor) {
        ...
        ajaxMensajeAdmin(text, emailReceptor);
    }  

    function ajaxMensajeAdmin(texto, emailReceptor) {
        $.ajaxSetup({
            headers: {
        ...
        $.ajax({
            url: "{{ url('/') }}/publico/rest/chat/comment/admin",
            type: 'POST',
            data: JSON.stringify({
                message: texto, 
            ...
            error: function(xhr, ajaxOptions, thrownError) {
                console.log(xhr);
                ajaxMensajeAdmin(texto, emailReceptor);
            }
        });
    }


    function getMensajesAdmin(idUSer, emailElemento, row, dialog) {
        $.ajax({
            url: "{{ url('/') }}/publico/rest/chat/comment/admin/" + idUSer + "/" + emailElemento + "/" + row,
            type: 'GET',
            contentType: "application/json", //TIPO DE TEXTO QUE SE ENVIA
            dataType: "text",    
            success: function(data) {
    ...
    function sendMensajeLeido(id) {
        $.ajax({
            url: "{{ url('/') }}/publico/rest/chat/comment/leido/" + id,
            type: 'GET',
            contentType: "application/json", //TIPO DE TEXTO QUE SE ENVIA
            dataType: "text",    
            success: function(data) {
