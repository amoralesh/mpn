...
namespace App\Http\Controllers\Administracion\Establecimientos;
...
use App\Daos\PuertasMercadoDao;
...
class MainController extends Controller
{
    ...
    public function __construct(PlazaDao $PlazaDao,
        ...
        PuertasMercadoDao  $PuertasMercadoDao,
    ...
    { 
        ...
        $this->puertasMercadoDao        = $PuertasMercadoDao;
    ...
    {
        return view('administracion.establecimientos.index');
    }

    public function dispositivosPruebas()
    {
        return view('administracion.establecimientos.negociosPruebas');
    }
    ...
    public function create()     
    {
        ...
        return view('administracion.establecimientos.create', $data);
    }
    ...
    public function edit($id)        
    {
        ...
        return view('administracion.establecimientos.edit', $data);
    } 
    ...
    {  
        ...
        if ( $validation->fails() ) {
            ...
            return Redirect::to("/administracion/establecimientos/create")->with('errores', 'Woops algo esta mal!.')->withErrors($validation)->withInput();
            ...
            $tipoNegocioE = $this->tipoNegocioDao->findById($tipoNegocio);
            ...
            $giroNegocioGeneralE = $this->giroNegocioGeneralDao->findById($giroNegocioGeneral);
            ...
            $seguimientos->setComentario("Se da de alta el establecimiento por el usuario: " . $usuario->getNombre() . " " . $usuario->getApellidoPaterno() . " "  . $usuario->getApellidoMaterno() );
            ...
            $dispositivoCreado = null;
            for ($i = 0; $i < count($tipoDispositivo); $i++) {
                if( $cantidad[$i] != "" || $cantidad[$i] != 0 ){
                    $tipoDispositivoE = $this->tipoDispositivoDao->findById($idTipoDispositivo[$i]);
                    $dispositivo = new Dispositivo;   
                    $dispositivo->setEtiqueta($tipoDispositivoE->getEtiqueta());
                    $dispositivo->setCantidad($cantidad[$i]);
                    $dispositivo->setToken(sha1($negocioCreado->getId() . $negocioCreado->getNombre() . $tipoDispositivoE->getId().$i."Negocio"));
                    $dispositivo->setNumeroActualizacion( "0" );
                    $dispositivo->setTipoDispositivo($tipoDispositivoE);
                    $dispositivo->setTipoStatus($tipoStatusE);
                    $dispositivo->setNegocio($negocioCreado);
                    $dispositivoCreado = $this->dispositivoDao->create($dispositivo); 
                }        
            }

            // VIEJA PLATAFORMA
            $result = $this->establecimientoDao->create( $negocioCreado , $dispositivoCreado );    
            if( $result ){  
                $idViejo = $this->establecimientoDao->findByToken( $dispositivoCreado->getToken() );
                $negocioCreado->setIdNegocio(  $idViejo['idEstablecimiento'] );
                $this->negocioDao->update( $negocioCreado );
            }
      
            $this->loggerDao->create(new Logger("Ha creado al establecimiento: " . $nombre));
            return Redirect::to("/administracion/establecimientos")->with('mensaje', 'Creado correctamente!.');
    ...
    public function update(Request $request, $id)
    {
        ...
        if ( $validation->fails() ) {
            //var_dump( $validation->messages() );
            return Redirect::to("/administracion/establecimientos/" . $id . "/edit" )->with('errores', 'Woops algo esta mal!.')->withErrors($validation)->withInput();
            ...
            $negocioActualizado = $this->negocioDao->update($negocio);
             ...
             $usuario = $this->usuarioDao->findByUser( session('usuario') );
             $seguimientos = new Seguimientos;
             $seguimientos->setComentario("Se actualizo el establecimiento por el usuario: " . $usuario->getNombre() . " " . $usuario->getApellidoPaterno() . " "  . $usuario->getApellidoMaterno() );
            ...
            // ------------------ TIPO DE DISPOSITIVOS ---------------------- idDispositivo
            for ($i = 0; $i < count($idDispositivo); $i++) {
 
                $tipoDispositivoE = $this->tipoDispositivoDao->findById( $idTipoDispositivo[$i] );  

                // SI EL DISPOSITIVO NO EXISTE Y HAY CANTIDAD ENTONCES SE CREA UN NUEVO DISPOSITIVO 
                 if( ( $idDispositivo[$i] == "" || $idDispositivo[$i] == 0 ) && ( intval( $cantidad[$i] ) != 0 || intval( $cantidad[$i] ) != "" ) ){ 

                    $dispositivo = new Dispositivo;   
                    $dispositivo->setEtiqueta( $tipoDispositivoE->getEtiqueta());
                    $dispositivo->setCantidad( $cantidad[$i] );
                    $dispositivo->setToken(sha1( $negocioActualizado->getId() . $negocioActualizado->getNombre() . $tipoDispositivoE->getId() . $i . "Actualizado"));
                    $dispositivo->setNumeroActualizacion( "0" );
                    $dispositivo->setTipoDispositivo( $tipoDispositivoE);
                    $dispositivo->setTipoStatus( $tipoStatusE);
                    $dispositivo->setNegocio( $negocio);
                    $this->dispositivoDao->create( $dispositivo);
                }
                // SI EL DISPOSITIVO EXISTE ENTONCES ACTUALIZA
                else if( ( $idDispositivo[$i] != "" || $idDispositivo[$i] != 0 ) && ( intval( $cantidad[$i] ) != 0 ||  intval( $cantidad[$i] ) != "" ) ){ 
                    
                    $dispositivo = $this->dispositivoDao->findById( $idDispositivo[$i] );
                    $dispositivo->setCantidad( $cantidad[$i] );
                    //$dispositivo->setNumeroActualizacion( ( intval( $dispositivo->getNumeroActualizacion() ) + 1 ) );
                    $this->dispositivoDao->update( $dispositivo);
                }

            }

            $this->loggerDao->create(new Logger("Ha actualizado al establecimiento: " . $nombre));
            return Redirect::to("/administracion/establecimientos/" . $id . "/edit" )->with('mensaje', 'actualizado correctamente!.');
        }
    }
    ...
    *===========================================================================================================================================*/
    public function listAll(Request $request)
    {
        ...
        $data = $this->negocioDao->listAllJson( $draw,  $maxResult , $firstResult , $buscar );
        return response( $data , 200 )->header('Content-Type', 'application/json');
    }


    public function listAllPruebas(Request $request)
    {
        $firstResult = $request->get('start');
        $maxResult   = $request->get('length');
        $draw        = $request->get('draw');
        $buscar      = $request->get('search')['value'];
        $data = $this->negocioDao->listAllModoPruebaDispositivosJson( $draw,  $maxResult , $firstResult , $buscar );
        return response( $data , 200 )->header('Content-Type', 'application/json');
    }
    ...
    { 
        ...
        $arreglo = array(
            "id"     => $negocio->getId(),
            "placaMpn"  => $negocio->getPlacaMPN() == 0 ? "Tiene" : ($negocio->getPlacaMPN() == 1 ? "NO TIENE":"NO SE SABE"),
            ...
            "comentarios" => $negocio->getComentarios(),     
            ...
            "statusRevisionNegocio" => $negocio->getStatusRevisionNegocio() == null ? "No se ha revisado" : $negocio->getStatusRevisionNegocio()->getEtiqueta(),   
            "codigoAguila"  => $negocio->getCodigoAguila() ? "Tiene" : "No Tiene",
            ...
            "sector" => $negocio->getSector() == null ? "Sin sector" : $negocio->getSector()->getEtiqueta() ,
    ...
    public function deshabilitar(Request $request)
    {
        ...
        $this->loggerDao->create( new Logger("Ha deshabilitado al establecimiento: ". $negocio->getNombre()." con el ID: ".$negocio->getId() ) );
    ...
    public function habilitar(Request $request)
    {
        ...
        $this->negocioDao->update( $negocio );
        $this->loggerDao->create( new Logger("Ha habilitado al establecimiento: ". $negocio->getNombre()." con el ID: ".$negocio->getId() ) );
    ...
    {  
        $idDispositivo = $request->get('id');

        $dispositivo = $this->dispositivoDao->findById($idDispositivo);      
        $negocio = $dispositivo->getNegocio(); 
        $tokenViejo = $dispositivo->getToken();

        if( $negocio == null ){//ES UN MERCADO
            $puertaMercado = $dispositivo->getPuertaMercado();   
            $negocio = $puertaMercado->getNegocio();     
        }        
        
        $tipoDispositivo = $dispositivo->getTipoDispositivo();

        if( $dispositivo->getNumeroActualizacion() == null || $dispositivo->getNumeroActualizacion() == 0  ){
        ...
        $dispositivo = $this->dispositivoDao->update( $dispositivo );
        $this->loggerDao->create(new Logger("Ha renovado al dispositivo: " . $dispositivo->getEtiqueta() . " con el token: " . $tokenViejo ));
        return response( 200 , 200)->header('Content-Type', 'application/json');   
    }


    public function deshabilitarDispositivo( Request $request )
    {  
        $idDispositivo = $request->get('id');
        $dispositivo = $this->dispositivoDao->findById($idDispositivo);
        $dispositivo->setStatus(false);
        $dispositivo = $this->dispositivoDao->update( $dispositivo );
        $this->loggerDao->create(new Logger("Ha deshabilitado al dispositivo: " . $dispositivo->getEtiqueta()  ));
        return response( 200 , 200)->header('Content-Type', 'application/json');   
    }


    public function habilitarDispositivo( Request $request )
    {  
        $idDispositivo = $request->get('id');
        $dispositivo = $this->dispositivoDao->findById($idDispositivo);
        $dispositivo->setStatus(true);
        $dispositivo = $this->dispositivoDao->update( $dispositivo );
        $this->loggerDao->create(new Logger("Ha habilitado al dispositivo: " . $dispositivo->getEtiqueta()  ));
        return response( 200 , 200)->header('Content-Type', 'application/json');   
    }

    public function dispositivoAPruebas( Request $request )
    {  
        $idDispositivo = $request->get('id');
        $dispositivo = $this->dispositivoDao->findById($idDispositivo);   
        
        $tipoStatusE = $this->tipoStatusDao->findById(5);
        $dispositivo->setTipoStatus($tipoStatusE);

        $dispositivo = $this->dispositivoDao->update( $dispositivo );
        $this->loggerDao->create(new Logger("Ha mandado a pruebas al dispositivo: " . $dispositivo->getEtiqueta()  ));
        return response( 200 , 200)->header('Content-Type', 'application/json');   
    }

    public function dispositivoAReal( Request $request )
    {  
        $idDispositivo = $request->get('id');
        $dispositivo = $this->dispositivoDao->findById($idDispositivo);   
        ...
        $dispositivo = $this->dispositivoDao->update( $dispositivo );
        $this->loggerDao->create(new Logger("Ha mandado a real al dispositivo: " . $dispositivo->getEtiqueta()  ));
        return response( 200 , 200)->header('Content-Type', 'application/json');   
    }
