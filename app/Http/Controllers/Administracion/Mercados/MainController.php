...
namespace App\Http\Controllers\Administracion\Mercados;
...
class MainController extends Controller
{
    ...
    {
        return view('administracion.mercados.index');
    }
    ...
    public function create()     
    {
        ...
        $data['tipoNegocioList']           = $this->tipoNegocioDao->listAllMercados();
        $data['giroNegocioList']           = $this->giroNegocioDao->listAll();
        $data['tipoDispositivoList']       = $this->tipoDispositivoDao->listAll();
        $data['giroNegocioGeneralList']    = $this->giroNegocioGeneralDao->listAllMercado();
        $data['statusRevisionNegocioList'] = $this->statusRevisionNegocioDao->listAll();
        return view('administracion.mercados.create', $data);
    }
    ...
    public function edit($id)        
    {
        ...
        return view('administracion.mercados.edit', $data);
    }
    ...
    {  
        ...
        if ( $validation->fails() ) {
            ...
            return Redirect::to("/administracion/mercados/create")->with('errores', 'Woops algo esta mal!.')->withErrors($validation)->withInput();
            ...
            $tipoNegocioE = $this->tipoNegocioDao->findById(7);
            ...
            $giroNegocioGeneralE = $this->giroNegocioGeneralDao->findById(1);
            ...
            $seguimientos->setComentario("Se da de alta al mercado por el usuario: " . $usuario->getNombre() . " " . $usuario->getApellidoPaterno() . " "  . $usuario->getApellidoMaterno() );
            ...
            /*=============PUERTAS MERCADO===============
            ============================================*/
            $arrayPuertasMercado = array();
            $length              = count($nombrePuerta); 
            for ($i = 0; $i < $length; $i++) {
                
                $puertaMe = new PuertasMercado;
                $puertaMe->setNombre($nombrePuerta[$i]);
                $puertaMe->setLatitudPuerta($latitudPuerta[$i]);
                $puertaMe->setLongitudPuerta($longitudPuerta[$i]);
                ...
                $direccion->setColonia($coloniaE);
                $direccion->setCodigoPostal($codigoPostal);
                $puertaMe->setDireccion($direccion);
                ...
                $dispositivo->setEtiqueta($tipoDispositivoE->getEtiqueta());
                $dispositivo->setTipoDispositivo($tipoDispositivoE);
                $dispositivo->setToken(sha1($negocioCreado->getId() . $negocioCreado->getNombre() . $tipoDispositivoE->getId().$i."Mercado"));
                $tipoStatusE = $this->tipoStatusDao->findById(1);
                $dispositivo->setTipoStatus($tipoStatusE);
               
                
                $puertaMe->setDispositivos($dispositivo);
                $puertaMe->setNegocio($negocioCreado);
                $arrayPuertasMercado[] = $puertaMe;
                $this->puertasMercadoDao->create($puertaMe); 
            }
            ...
            $this->loggerDao->create(new Logger("Ha creado al mercado: " . $nombre));
            return Redirect::to("/administracion/mercados")->with('mensaje', 'Creado correctamente!.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ( $validation->fails() ) {
            //var_dump( $validation->messages() );
            return Redirect::to("/administracion/mercados/" . $id . "/edit" )->with('errores', 'Woops algo esta mal!.')->withErrors($validation)->withInput();
           ...
           /* for ($i = 0; $i < count($idDispositivo); $i++) {
            ...
            $puertasRecuperadas  = $negocio->getPuertaMercado();
            $lengPuertas         = count($puertasRecuperadas);
            $arrayPuertasMercado = array();
            
            foreach ($puertasRecuperadas as $i => $puertaMe) {
                
                $puertaMe->setNombre($nombrePuertaNueva[$i]);
                $puertaMe->setLatitudPuerta($latitudPuertaNueva[$i]);
                $puertaMe->setLongitudPuerta($longitudPuertaNueva[$i]);
                
                $direccion = $puertaMe->getDireccion();
                ...
                $dispositivo = $puertaMe->getDispositivos();
                $dispositivo->setEtiqueta($tipoDispositivoE->getEtiqueta());
                $dispositivo->setTipoDispositivo($tipoDispositivoE);
                ...
                $puertaMe->setDispositivos($dispositivo);
                $puertaMe->setNegocio($negocio);
                $arrayPuertasMercado[] = $puertaMe;
                
                $this->puertasMercadoDao->update($puertaMe);
            ...
            if ($nombrePuerta > 0) {
                ...
                for ($i = 0; $i < $length; $i++) {
                    ...
                    $dispositivo->setTipoDispositivo($tipoDispositivoE);
                    $dispositivo->setNumeroActualizacion( "0" );
                    $dispositivo->setToken(sha1($negocio->getId() . $negocio->getNombre() . $tipoDispositivoE->getId().$i."Mercado"));
                    $tipoStatusE = $this->tipoStatusDao->findById(1);
                    $dispositivo->setTipoStatus($tipoStatusE);
          ...
          //END 
            ...
            $this->loggerDao->create(new Logger("Ha actualizado al mercado: " . $nombre));
            return Redirect::to("/administracion/mercados/" . $id . "/edit" )->with('mensaje', 'actualizado correctamente!.');
        } /*==END ELSE==*/
    }/*==UPDATE==*/
    ...
    *===========================================================================================================================================*/
    ...
    /**  VALIDADO */
    public function listAllJsonMercados(Request $request)
    {
        $firstResult = $request->get('start');
        $maxResult   = $request->get('length');
        $draw        = $request->get('draw');
        $buscar      = $request->get('search')['value'];
        $data = $this->negocioDao->listAllJsonMercados( $draw,  $maxResult , $firstResult , $buscar );
    ...
    { 
        ...
        // DISPOSITIVOS DEL NEGOCIO   
        foreach( $negocio->getPuertaMercado() as $indice => $puerta ){
    ...
    public function deshabilitar(Request $request)
    {
        ...
        $this->loggerDao->create( new Logger("Ha deshabilitado al Mercado: ". $negocio->getNombre()." con el ID: ".$negocio->getId() ) );
    ...
    public function habilitar(Request $request)
    {
        ...
        $this->negocioDao->update( $negocio );
        $this->loggerDao->create( new Logger("Ha habilitado al Mercado: ". $negocio->getNombre()." con el ID: ".$negocio->getId() ) );
