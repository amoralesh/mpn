...
namespace App\Http\Controllers\Administracion\Escuelas;
...
class MainController extends Controller
{
    ...
    { 
        ...
        $this->middleware('hasPermission:Administracion:Aside:Escuelas',[ 'only' => ['index' ]] );
        $this->middleware('hasPermission:Administracion:Escuelas:Crear',[ 'only' => ['create' ]] );
        $this->middleware('hasPermission:Administracion:Escuelas:Editar',[ 'only' => ['edit' ]] );
        $this->middleware('hasPermission:Administracion:Escuelas:Guardar',[ 'only' => ['store' ]] );
        $this->middleware('hasPermission:Administracion:Escuelas:Actualizar',[ 'only' => ['update' ]] );

        $this->middleware('hasPermission:Administracion:Escuelas:Habilitar',[ 'only' => ['habilitar' ]] );
        $this->middleware('hasPermission:Administracion:Escuelas:Deshabilitar',[ 'only' => ['deshabilitar' ]] );

        $this->middleware('hasPermission:Administracion:Rest:Escuelas',[ 'only' => [ 'listAll' ]] );
        $this->middleware('hasPermission:Administracion:Escuelas:Actualizar:Token',[ 'only' => ['actualizarToken' ]] );
    ...
    public function create()     
    {
        ...
        $data['tipoNegocioList']           = $this->tipoNegocioDao->listAllEscuela();
        $data['giroNegocioList']           = $this->giroNegocioDao->listAllEscuelas();
        $data['tipoDispositivoList']       = $this->tipoDispositivoDao->listAll();
        $data['giroNegocioGeneralList']    = $this->giroNegocioGeneralDao->listAllEscuela();
        $data['statusRevisionNegocioList'] = $this->statusRevisionNegocioDao->listAll();
        return view('administracion.escuelas.create', $data);
    }
    ...
    public function edit($id)        
    {
        ...
        return view('administracion.escuelas.edit', $data);
    } 
    ...
    {  
        ...
        if ( $validation->fails() ) {
            ...
            return Redirect::to("/administracion/escuelas/create")->with('errores', 'Woops algo esta mal!.')->withErrors($validation)->withInput();
            ...
            //id 16 = ESCUELASServicios educativos
            $tipoNegocioE = $this->tipoNegocioDao->findById(16);
            ...
            //id 2 = ESCUELAS
            $giroNegocioGeneralE = $this->giroNegocioGeneralDao->findById(2);
            ...
            return Redirect::to("/administracion/escuelas")->with('mensaje', 'Creado correctamente!.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ( $validation->fails() ) {
            //var_dump( $validation->messages() );
            return Redirect::to("/administracion/escuelas/" . $id . "/edit" )->with('errores', 'Woops algo esta mal!.')->withErrors($validation)->withInput();
            ...
            */
            ...
            return Redirect::to("/administracion/escuelas/" . $id . "/edit" )->with('mensaje', 'actualizado correctamente!.');
        }
    }
    ...
    *===========================================================================================================================================*/
    ...
    public function listAll(Request $request)
    {
        ...
        $data = $this->negocioDao->listAllEscuelasJson( $draw,  $maxResult , $firstResult , $buscar );
    ...
    { 
        ...
        foreach( $negocio->getDispositivos() as $indice => $dispositivo ){
            ...
            //PRUEBAS
            $pruebasArreglo = array();   
            foreach( $dispositivo->getPruebas() as $indice => $prueba ){
                $arreglo = array(          
                    "id" => $prueba->getId(),
                    "contenido" => $prueba->getContenido(),
                    "fechaAlta" => $prueba->getFechaAlta()
                );
                $pruebasArreglo[] = $arreglo;
            } 

            //ALERTAS
            $alertasArreglo = array();   
            foreach( $dispositivo->getAlertas() as $indice => $alerta ){
                $arreglo = array(      
                    "id" => $alerta->getId(),
                ...
                $alertasArreglo[] = $arreglo;
            } 

            //"usuario" => $dispositivo->getPuertasMercado(),
            ...
            $arreglo = array(    
              ...
              "tipoStatus" => $dispositivo->getTipoStatus(),    
              "numeroActualizacion" => $dispositivo->getNumeroActualizacion(),
              "pruebas" => $pruebasArreglo ,
              "alertas" => $alertasArreglo ,
    ...
    public function actualizarToken(Request $request){
        ...
        if( $isMercado == null ){//ES UN MERCADO
            $puertaMercado = $dispositivo->getPuertasMercado();   
